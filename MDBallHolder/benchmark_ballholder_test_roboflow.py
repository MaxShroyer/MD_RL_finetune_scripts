"""Benchmark Ball-Holder test split with a hosted Roboflow model.
...
DEFAULT_MODEL_ID = "maxs-m87-ball-holder-splits-v1-072d328f-zny9c/1"
DEFAULT_DATASET_DIR = "out_hf_yolo/maxs-m87_Ball-Holder-splits-v1/test"
DEFAULT_IMAGE_DIR = os.path.join(DEFAULT_DATASET_DIR, "images")
DEFAULT_LABEL_DIR = os.path.join(DEFAULT_DATASET_DIR, "labels")
...
def _parse_yolo_label_file(label_path: str, allowed_class_ids: Optional[set[int]]) -> List[Box]:
...
def _iter_test_samples(
    *,
    ...
    for name in image_names:
        ...
        boxes = _parse_yolo_label_file(label_path, allowed_class_ids)
        if max_boxes is not None:
            boxes = boxes[:max_boxes]
        yield image, boxes, name
...
def _parse_class_ids(raw: Optional[str]) -> Optional[set[int]]:
    if not raw:
        return None
    items = [token.strip() for token in raw.replace(";", ",").split(",") if token.strip()]
    if not items:
        return None
    class_ids = set()
    for item in items:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark Ball-Holder test split with Roboflow hosted inference.")
    ...
    parser.add_argument("--model-id", default=os.environ.get("ROBOFLOW_MODEL_ID", DEFAULT_MODEL_ID))
    parser.add_argument("--project", default=os.environ.get("ROBOFLOW_PROJECT"))
    parser.add_argument("--version", type=int, default=None)
    parser.add_argument("--image-dir", default=_repo_relative(DEFAULT_IMAGE_DIR))
    parser.add_argument("--label-dir", default=_repo_relative(DEFAULT_LABEL_DIR))
    parser.add_argument("--class-name", default=os.environ.get("ROBOFLOW_CLASS_NAME"))
    parser.add_argument(
        "--allowed-class-ids",
        default=None,
        help="Comma-separated class ids to include from YOLO labels (default: all).",
    )
    ...
    parser.add_argument("--viz-dir", default=_repo_relative("outputs/test_viz/roboflow"))
    parser.add_argument("--viz-samples", type=int, default=25)
    parser.add_argument("--metrics-json", default=_repo_relative("outputs/test_metrics_roboflow.json"))
    ...
    allowed_class_ids = _parse_class_ids(args.allowed_class_ids)
    ...
    if args.metrics_json:
        output_payload = {
            ...
            "config": {
                ...
                "label_dir": args.label_dir,
                "class_name": args.class_name,
                "allowed_class_ids": sorted(allowed_class_ids) if allowed_class_ids else None,
                "iou_threshold": args.iou_threshold,
                "max_samples": args.max_samples,
                "max_boxes": args.max_boxes,
                "confidence": args.confidence,
