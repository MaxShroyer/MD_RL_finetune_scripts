...
def _save_checkpoint(*, finetune: Any, context: str) -> bool:
    try:
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    try:
        ...
        if args.eval_every > 0:
            ...
            if args.save_on_eval and _save_checkpoint(finetune=finetune, context=f"baseline eval step={args.resume_step}"):
        ...
        for step in step_iter:
            ...
            if args.eval_every > 0 and (global_step + 1) % args.eval_every == 0:
                ...
                if best_metric_value is None or metric_value > best_metric_value:
                    ...
                    elif _save_checkpoint(finetune=finetune, context=f"best metric checkpoint step={global_step}"):
                        print(f"new best {args.best_metric}={metric_value:.4f} at step {global_step}; checkpoint saved")
                if args.save_on_eval and _save_checkpoint(finetune=finetune, context=f"periodic eval step={global_step}"):
            ...
            if args.save_every > 0 and (global_step + 1) % args.save_every == 0:
                _save_checkpoint(finetune=finetune, context=f"save_every checkpoint step={global_step}")

        _save_checkpoint(finetune=finetune, context="final checkpoint save")
