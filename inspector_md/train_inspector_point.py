#!/usr/bin/env python3
"""Thin wrapper around the shared point RL trainer for Inspector MD."""
...
DEFAULT_CONFIG_PATH = repo_relative("configs", "train_inspector_point_default.json")
...
def parse_args(argv: Optional[list[str]] = None):
    ...
    if not args.finetune_id and str(args.finetune_name).startswith("pid-icons-"):
        args.finetune_name = f"inspector-point-{_base._random_suffix()}"
    args.async_checkpoint_eval_benchmark_script = str((SCRIPT_DIR / "benchmark_inspector_pipeline.py").resolve())
    return args
