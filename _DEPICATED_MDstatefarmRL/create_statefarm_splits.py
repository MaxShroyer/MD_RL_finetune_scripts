...
def _build_hard_limited_splits(
    ...
    if len(annotated_rows) < ann_target or len(unannotated_rows) < unann_target:
        raise ValueError(
            ...
            f"Needed ann/unann={ann_target}/{unann_target}, found {ann_seen}/{unann_seen}."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create splits with post-training holdout.")
    parser.add_argument("--dataset", required=True, help="HF dataset name, e.g. maxs-m87/NBA_Statefarm_01")
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0 / 3.0,
        help="Fraction of each split that should be empty/unannotated.",
    )
    parser.add_argument(
        "--hard-sample-limit",
        type=int,
        ...
        help=(
            ...
            "Uses streaming mode to avoid full dataset cache writes and auto-adjusts "
            "holdout when too large for tiny splits."
        ),
    )
