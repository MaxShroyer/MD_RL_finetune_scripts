"""Detect finetuning example for State Farm logo detection.
...
STATEFARM_DATASET = "maxs-m87/NBA_StateFarm_Splits_1"
...
def _parse_statefarm_val_boxes(answer: Optional[list]) -> List[DetectAnnotation]:
    if not answer:
        return []
    boxes: List[DetectAnnotation] = []
    for item in answer:
    ...
    *,
    ...
    while True:
        ...
        for row in ds:
            ...
            boxes = _parse_statefarm_boxes(row.get("answer_boxes"), width, height)
            if max_boxes is not None:
                boxes = boxes[:max_boxes]
            yield Sample(image=image, boxes=boxes, object_name=OBJECT_NAME, source="statefarm")
...
def _augment_sample(
    ...
    *,
    ...
    if rng.random() < config.crop_p:
        ...
        # Avoid converting positive samples into empty-label negatives via crop.
...
def main() -> None:
    ...
    parser.add_argument("--wandb-project", default="tuna-statefarm-detect")
    ...
    parser.add_argument("--val-image-dir", default="val")
    parser.add_argument(
        "--best-metric",
    ...
    statefarm_stream = _stream_statefarm_samples(
    ...
    if not args.finetune_id and not args.finetune_name:
        args.finetune_name = f"statefarm-detect-{_random_suffix()}"
    ...
    run = wandb.init(
        ...
        config={
            ...
            "lora_rank": args.rank,
            "statefarm_dataset": args.dataset,
            "split": args.split,
            "train_split": train_split,
            "val_split": val_split,
            "seed": args.seed,
