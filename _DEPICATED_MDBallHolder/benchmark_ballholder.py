...
def _box_to_dict(box: Box) -> dict[str, float]:
    return {
...
def _sample_record(
    *,
    ...
    return {
        ...
        "pred_count": None if pred_boxes is None else len(pred_boxes),
        "ground_truth_boxes": [_box_to_dict(box) for box in gt_boxes],
        "predicted_boxes": None if pred_boxes is None else [_box_to_dict(box) for box in pred_boxes],
        "task_f1": f1,
        "task_miou": miou,
        "tp": tp,
        "fp": fp,
...
def _build_before_after_payload(
    *,
    args: argparse.Namespace,
    baseline_metrics: Optional[dict[str, object]],
    candidate_metrics: Optional[dict[str, object]],
    baseline_records: List[dict[str, object]],
    candidate_records: List[dict[str, object]],
) -> dict[str, object]:
    baseline_by_id = {str(record["sample_id"]): record for record in baseline_records}
    candidate_by_id = {str(record["sample_id"]): record for record in candidate_records}

    ordered_ids: List[str] = []
    for record in baseline_records:
        sample_id = str(record["sample_id"])
        if sample_id not in ordered_ids:
    ...
    for sample_id in ordered_ids:
        ...
        after = candidate_by_id.get(sample_id)
        reference = before or after or {}
        samples.append(
            {
                "sample_index": reference.get("sample_index"),
                "sample_id": sample_id,
                "ground_truth_boxes": reference.get("ground_truth_boxes"),
                "before": before,
                "after": after,
            }
        )
    ...
    try:
        if finetune is None:
            ...
            for image, gt_boxes, sample_id in samples_iter:
                ...
                batch_indexes.append(sample_index)
                ...
                try:
                    ...
                    for gt_boxes_item, sample_id_item, sample_index_item in zip(batch_gt, batch_ids, batch_indexes):
                        sample_records.append(
                            _sample_record(
                                sample_index=sample_index_item,
                ...
                for pred_boxes, gt_boxes_item, sample_id_item, image_item, sample_index_item in zip(
                    batch_pred, batch_gt, batch_ids, batch_images, batch_indexes
                ):
                    total += 1
                ...
                batch_images, batch_gt, batch_ids, batch_indexes = [], [], [], []
...
def main() -> None:
    ...
    parser.add_argument(
        "--before-after-json",
        default="",
        help="Optional JSON path for per-sample baseline/candidate comparison records.",
    )
    ...
    if not args.skip_baseline:
        ...
        baseline_metrics, baseline_records = _evaluate_model(
            label="baseline",
            args=args,
    ...
    if has_candidate:
        ...
        candidate_metrics, candidate_records = _evaluate_model(
            label="candidate",
            args=args,
            model=args.model,
    ...
    if str(args.before_after_json).strip():
        before_after_payload = _build_before_after_payload(
            args=args,
        ...
        os.makedirs(os.path.dirname(args.before_after_json) or ".", exist_ok=True)
        with open(args.before_after_json, "w", encoding="utf-8") as handle:
            json.dump(before_after_payload, handle, indent=2)
        print(f"saved sample comparison -> {args.before_after_json}")
