#!/usr/bin/env python3
"""Thin wrapper around the shared detect RL trainer for local aerial datasets."""
...
from aerial_airport.common import DEFAULT_STAGING_API_BASE, DETECT_PROMPT_OVERRIDES, detect_prompt_for_class, repo_relative
...
def _prompt_for_class(class_name: str, *, style: str = "detect_phrase") -> str:
    return detect_prompt_for_class(class_name, prompt_overrides=DETECT_PROMPT_OVERRIDES)
...
def parse_args(argv: Optional[list[str]] = None):
    ...
    if not args.finetune_id and str(args.finetune_name).startswith("pid-icons-"):
        args.finetune_name = f"visdrone-vid-detect-{_base._random_suffix()}"
