#!/usr/bin/env python3
"""Benchmark ConstructionSite rule-VQA query models."""
...
from construction_site import train_construction_site_query_rule_vqa as train_utils  # noqa: E402

DEFAULT_BASE_URL = "https://api-staging.moondream.ai/v1"
DEFAULT_CONFIG_PATH = repo_relative("configs", "benchmark_construction_site_query_rule_vqa_default.json")
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Benchmark ConstructionSite rule-VQA query models.")
    parser.add_argument("--config", default=str(config_path))
    parser.add_argument("--env-file", default=str(repo_relative(".env.staging")))
    parser.add_argument("--api-key", default="")
    parser.add_argument("--api-key-env-var", default="CICID_GPUB_MOONDREAM_API_KEY_2")
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    hallucinated_values: list[int] = []
    strict_hallucinated_values: list[int] = []
    ...
    try:
        for index in query_common.tqdm(
            ...
            hallucinated_values.append(int(outcome.hallucinated_rule_count))
            strict_hallucinated_values.append(int(outcome.strict_hallucinated_rule_count))
    ...
    metrics = {
        ...
        "eval_positive_samples": positive_total,
        ...
        "eval_hallucinated_rules_mean": fmean(float(value) for value in hallucinated_values) if hallucinated_values else 0.0,
        "eval_strict_hallucinated_rules_mean": (
            fmean(float(value) for value in strict_hallucinated_values) if strict_hallucinated_values else 0.0
        ),
